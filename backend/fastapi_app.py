...
def _read_config_from_repo(package_name: str, github_token: str, repository_info: Optional[Dict] = None) -> Dict[str, str]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Extract testbed: "designed for testbed [_number_]" or "designed for testbed number"
            # Pattern matches: "testbed [_1_]" or "testbed [1]" or "testbed 1" or "testbed-1"
            testbed_pattern = r'designed\s+for\s+testbed\s+\[?([^\]]+)\]?|testbed\s+\[?([^\]]+)\]?'
            testbed_match = re.search(testbed_pattern, readme_content, re.IGNORECASE)
            if testbed_match:
                testbed_value = testbed_match.group(1) if testbed_match.group(1) else testbed_match.group(2)
                ...
                if testbed_num:
                    config['testbed'] = f"testbed-{testbed_num.group(1)}"
            
            # Extract use case: "for the [_your use case_] use case"
            # Pattern matches: "for the [_something_] use case" or "for the [something] use case"
            usecase_pattern = r'for\s+the\s+\[([^\]]+)\]\s+use\s+case'
            usecase_match = re.search(usecase_pattern, readme_content, re.IGNORECASE)
            if usecase_match:
                config['usecase'] = usecase_match.group(1).strip()
            
            # Extract developer: "It was developed by [_your organization's name_]."
            # Pattern matches: "developed by [_something_]" or "developed by [something]"
            developer_pattern = r'(?:It\s+was\s+)?developed\s+by\s+\[([^\]]+)\]'
            developer_match = re.search(developer_pattern, readme_content, re.IGNORECASE)
            if developer_match:
                config['developer'] = developer_match.group(1).strip()
